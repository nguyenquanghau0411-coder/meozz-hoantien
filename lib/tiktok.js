...
/**
 * ==================== ĐƠN HÀNG TIKTOK (RioHub) ====================
 * Port 1:1 từ riohub_fetch_orders() / _riohub_order_commission() /
 * _riohub_order_status_label() trong bot_v48_spf.py (lệnh #donhangtik) —
 * để web cũng lấy được đơn hàng TikTok trực tiếp từ RioHub, không cần
 * chờ đồng bộ qua file JSON nào cả.
 */

// Cùng công thức quy đổi hoa hồng đang dùng trong bot cho #donhangtik/#vitientik:
// hoa hồng gốc → trừ 11% thuế → nhân 80% chia cho khách.
const TIKTOK_PLATFORM_FEE_RATE = 0.11;
const TIKTOK_PAYOUT_SHARE = 0.8;
const TIKTOK_NET_MULTIPLIER = (1 - TIKTOK_PLATFORM_FEE_RATE) * TIKTOK_PAYOUT_SHARE; // 0.712

const RIOHUB_STATUS_LABEL = { 1: "Đang chờ xử lý", 2: "Hoàn thành", 3: "Đã huỷ/hoàn tiền" };

// Cache TTL ngắn theo sub_id (giống bot) — tránh dội request vào RioHub khi
// khách vào lại dashboard liên tục (giới hạn RioHub 300 req/phút/key).
const _riohubOrdersCache = new Map(); // subId -> { fetchedAt, orders }
const RIOHUB_ORDERS_CACHE_TTL_MS = 5 * 60 * 1000; // 5 phút

/** GET /partner/tiktok/affiliate/orders?creator_username=...&sub1=<subId>,
 * tự lấy hết trang nếu total > page_size, retry khi 429 (tôn trọng Retry-After).
 * Trả về [] nếu lỗi/không có đơn (không throw, để không làm sập trang dashboard). */
export async function fetchTiktokOrders(subId, pageSize = 200) {
  if (!RIOHUB_API_KEY || !subId) return [];

  const cached = _riohubOrdersCache.get(subId);
  if (cached && Date.now() - cached.fetchedAt < RIOHUB_ORDERS_CACHE_TTL_MS) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      const res = await fetch(`${RIOHUB_BASE_URL}/partner/tiktok/affiliate/orders?${params}`, {
        headers: { "X-Riohub-Api-Key": RIOHUB_API_KEY, "User-Agent": "MeozzHoanTien/1.0" },
        signal: AbortSignal.timeout(10000),
      });

      if (res.status === 429) {
        const retryAfter = Math.min(parseFloat(res.headers.get("Retry-After") || "2") || 2, 5);
        await new Promise((r) => setTimeout(r, retryAfter * 1000));
        continue;
      }
      ...
      const data = await res.json().catch(() => ({}));
      if (res.status !== 200) {
        console.warn(`[fetchTiktokOrders] RioHub /orders lỗi ${res.status} (sub_id=${subId}):`, data);
      ...
      let curPage = data.page ?? 1;

      // Lấy nốt các trang còn lại nếu total vượt page_size (hiếm khi xảy ra).
      while (total > curPage * pageSizeResp) {
        curPage += 1;
        params.set("page", String(curPage));
        const res2 = await fetch(`${RIOHUB_BASE_URL}/partner/tiktok/affiliate/orders?${params}`, {
          headers: { "X-Riohub-Api-Key": RIOHUB_API_KEY, "User-Agent": "MeozzHoanTien/1.0" },
          signal: AbortSignal.timeout(10000),
        });
        if (res2.status !== 200) break;
        const data2 = await res2.json().catch(() => ({}));
        allOrders = allOrders.concat(data2.orders || []);
      }
      break;
    } catch (err) {
      console.warn(`[fetchTiktokOrders] lỗi kết nối (lần ${attempt}/${maxAttempts}, sub_id=${subId}):`, err?.message || err);
      if (attempt < maxAttempts) {
        await new Promise((r) => setTimeout(r, 1000));
        continue;
      }
  ...
  _riohubOrdersCache.set(subId, { fetchedAt: Date.now(), orders: allOrders });
  return allOrders;
}

/** Hoa hồng hiển thị cho khách = hoa hồng RioHub trả (actual, hoặc est nếu
 * chưa đối soát) × TIKTOK_NET_MULTIPLIER. Đơn "Đã huỷ/hoàn tiền" (status=3)
 * → luôn 0đ, bất kể RioHub trả actual/est_commission > 0 hay không. */
function riohubOrderCommission(order) {
  if (order?.status === 3) return 0;
  let raw = order?.actual_commission;
  if (raw === null || raw === undefined || raw === "") raw = order?.est_commission;
  const rawF = parseFloat(raw) || 0;
  return Math.round(rawF * TIKTOK_NET_MULTIPLIER * 100) / 100;
}

/** Nhãn trạng thái tiếng Việt + ngày hoàn thành (nếu có), khớp mẫu bot dùng
 * cho #donhangtik. */
function riohubOrderStatusLabel(order) {
  const status = order?.status;
  let label = RIOHUB_STATUS_LABEL[status] || order?.settlement_status || "—";
  if (status === 2) {
    const ts = order?.time_delivered || order?.time_created;
    if (ts) {
      const d = typeof ts === "number" ? new Date(ts * 1000) : new Date(ts);
      if (!Number.isNaN(d.getTime())) {
        const dd = String(d.getDate()).padStart(2, "0");
        const mm = String(d.getMonth() + 1).padStart(2, "0");
        label = `Hoàn thành(${dd}/${mm})`;
      }
    }
  }
  return label;
...
function riohubTimestampToIso(ts) {
  if (!ts) return null;
  const d = typeof ts === "number" ? new Date(ts * 1000) : new Date(ts);
  return Number.isNaN(d.getTime()) ? null : d.toISOString();
}

/** Chuẩn hoá orders[] thô từ RioHub sang đúng shape đang dùng cho đơn Shopee
 * (id/productName/commission/status/orderedAt/completedAt) + field
 * platform:"tiktok" để dashboard biết đây là đơn TikTok (hoa hồng đã quy
 * đổi sẵn theo TIKTOK_NET_MULTIPLIER, KHÔNG chạy lại qua commissionBreakdown
 * kiểu Shopee nữa). Sắp mới nhất trước, giống bot. */
export function normalizeTiktokOrders(rawOrders) {
  if (!Array.isArray(rawOrders)) return [];
  return [...rawOrders]
    .sort((a, b) => (b.create_time || 0) - (a.create_time || 0))
    .map((o) => {
      let rawCommission = o?.actual_commission;
      if (rawCommission === null || rawCommission === undefined || rawCommission === "") {
        rawCommission = o?.est_commission;
      }
      const grossCommission = o?.status === 3 ? 0 : parseFloat(rawCommission) || 0;
      return {
        id: o.order_id || "",
        productName: (o.product_name || "").trim() || "—",
        commission: riohubOrderCommission(o), // đã quy đổi (sau thuế 11% + 80%) — dùng làm "hoa hồng thực nhận"
        grossCommission, // hoa hồng gốc, chưa trừ gì — dùng cho cột "Hoa hồng"
        status: riohubOrderStatusLabel(o),
        orderedAt: riohubTimestampToIso(o.time_created || o.create_time),
        completedAt: riohubTimestampToIso(o.time_delivered),
        platform: "tiktok",
      };
    });
}
