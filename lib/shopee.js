...
// [Đồng bộ bot_v48_spf] Affiliate ID dùng chung cho Shopee + ShopeeFood/SPF.
const AFFILIATE_ID = "17395950528"; // ID affiliate Shopee của bạn
...
// [v44 bot_v48_spf] Link ShopeeFood/SPF: spf.shopee.vn/... hoặc
// shopeefood.shopee.vn/u/... — convert affiliate TRỰC TIẾP (bọc nguyên link
// gốc làm origin_link), KHÔNG resolve shop_id/product_id, KHÔNG tra hoa hồng
// (ShopeeFood không có API tra hoa hồng sản phẩm).
const SPF_LINK_RE = /https?:\/\/(?:spf\.shopee\.vn|shopeefood\.shopee\.vn\/u)\/[^\s"'<>)\]]+/i;

export function isSpfLink(text) {
  return SPF_LINK_RE.test(text || "");
}

export function extractSpfLink(text) {
  const m = SPF_LINK_RE.exec(text || "");
  if (!m) return null;
  return m[0].replace(/[.,;!?]+$/, "");
}
...
// [Đồng bộ bot_v48_spf._fetch_commission()] Hoa hồng Shopee KHÔNG dùng thẳng
// "shopeeComFinal" API trả về — bot LUÔN tính lại cố định 8% giá sản phẩm,
// cap tối đa 40.000đ, bất kể API trả bao nhiêu. Chỉ có hoa hồng Xtra
// ("sellerComFinal") là giữ nguyên số thật từ API, KHÔNG cap.
const SHOPEE_COMMISSION_RATE = 0.08;
const SHOPEE_COMMISSION_CAP = 40000;
...
async function fetchProductInfo(shopeeUrl) {
  try {
    ...
    if (!price) {
      // Không lấy được giá sản phẩm -> không đủ dữ liệu để tính hoa hồng,
      // giống bot: coi như "không có hoa hồng" thay vì hiện 0%/0đ sai lệch.
      return null;
    }

    // Hoa hồng Xtra (sellerComFinal) — giữ nguyên số thật từ API, không cap.
    const sellerCom = p.sellerComFinal || 0;
    // Hoa hồng Shopee — LUÔN tính lại 8% giá, cap 40.000đ (giống hệt bot),
    // KHÔNG dùng trực tiếp shopeeComFinal API trả về.
    const shopeeCom = Math.min(price * SHOPEE_COMMISSION_RATE, SHOPEE_COMMISSION_CAP);
    const sellerRate = sellerCom / price;

    const commission = sellerCom + shopeeCom;
    const commissionPct = (SHOPEE_COMMISSION_RATE + sellerRate) * 100;
    ...
    return {
      ...
      commissionPct: formatPct(commissionPct),
      image,
    };
  } catch {
...
/**
 * Chuyển 1 link ShopeeFood/SPF → affiliate link — giống hệt spf_convert() /
 * process_spf_link() trong bot_v48_spf.py: bọc THẲNG link gốc làm origin_link
 * trong an_redir, dùng CHUNG affiliate_id + sub_id (My ID) với Shopee thường.
 * KHÔNG resolve shop_id/product_id, KHÔNG gọi API hoa hồng (ShopeeFood không
 * có API tra hoa hồng sản phẩm) — nên productName/commission luôn rỗng.
 */
export async function convertSpfLink(rawUrl, myId) {
  const origin = (rawUrl || "").trim();
  if (!origin) {
    throw new Error("Link ShopeeFood không hợp lệ.");
  }
